...
onMounted(() => {
    ...
    if(data.profile_image != 0) {
        document.querySelector('#settings_profilePicture').src = '../uploads/'+data.profile_image;
    }
});
...
function processFiles(files) {
    if(files.length > 0) {
        ...
        data.user_uploads.forEach(upload => {
            files.forEach(file => {
                if(file == upload.id) {
        ...
        document.querySelector('#settings_profilePicture').src = `../uploads/${uploadsPreview[0]['name']}`;

        let currentObj = this;

        axios.post(`/settings/account/profile-picture`, {
        picture: files[0],
        })
